...
Использование реплики (Slave-сервера) дает финансовой компании несколько критических преимуществ:
    Минимальное RTO (время восстановления):
        Если основной сервер упал, переключение на реплику занимает секунды или минуты.
        Восстановление из бэкапа (особенно если база весит терабайты) может занять часы.

    Снятие нагрузки (Read Scaling):
        вые отчеты или делать аналитические выгрузки,
        не замедляя работу основного сервера,
        на котором клиенты совершают транзакции.

    Бэкап без падения производительности:
        Можно запускать mysqldump или копирование файлов прямо на реплике.
        При этом основной сервер (Master) не почувствует никакой нагрузки,
        и работа пользователей не замедлится.

    Защита от физических аварий:
        Если реплика находится в другом дата-центре,
        то пожар или наводнение в основном ЦОД не приведет к потере данных и остановке бизнеса.
```
# Важно помнить!
## Репликация не спасает от «человеческого фактора». Если пришел запрос DROP TABLE users, он мгновенно исполнится и на мастере, и на реплике. Поэтому реплика + регулярные бэкапы — это единственный надежный стандарт.
